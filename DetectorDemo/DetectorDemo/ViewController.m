...
/*
    ...
    NSPhotoLibraryUsageDescription 保存图片到相册需要这个
 */
- (void)cameraScanDocument:(UIViewController *)viewController{
    if (@available(iOS 13.0, *)) {
        if (!VNDocumentCameraViewController.supported) {
            ...
            self.dcVC = [[VNDocumentCameraViewController alloc] init];
            self.dcVC.delegate = self;
            [self presentViewController:self.dcVC animated:YES completion:^{
...
- (void)writeImage:(NSMutableArray<UIImage *> *)imgs{
    ...
    //获取我的相薄里面的所有相册
    PHFetchOptions *fetOptions = [[PHFetchOptions alloc] init];
    fetOptions.includeAssetSourceTypes = PHAssetSourceTypeUserLibrary;
    PHFetchResult *result = [PHCollectionList fetchTopLevelUserCollectionsWithOptions:fetOptions];
    [result enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        PHAssetCollection *assetCollection = obj;
        NSLog(@"localizedTitle = %@", assetCollection.localizedTitle);
        if ([assetCollection.localizedTitle isEqualToString:@"QQ"]) { //选定目标相册
            [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                ...
                if (success) { //保存图片成功
                    NSLog(@"保存图片成功");
                    [self.dcVC dismissViewControllerAnimated:YES completion:^{
...
//        [self writeImage:imageMtbAry];
        [VisionManager imageDetectionManager:imageMtbAry];
